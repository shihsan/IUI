...
- (void)viewDidLoad
{
    ...
    if ([self.navbar.title isEqualToString:@"☻基本資料"]) {
    ...
    else if ([self.navbar.title isEqualToString:@"☻學歷"]){
        self.line.image = [UIImage imageNamed:@"line_2.gif"];
        //self.photo.image = [UIImage imageNamed:@"bobo.jpg"];
        self.textview.text = @" ★國立政治大學資訊科學系（目前就讀）\n  -----------------------------------\n ★臺北市立中山女子高級中學\n  -----------------------------------\n ★臺北市市立中崙高中（附設國中部）\n  -----------------------------------\n ★臺北市信義區光復國小\n  -----------------------------------\n ★基督教台灣信義會蒙恩堂幼稚園";
    }
    else if ([self.navbar.title isEqualToString:@"☻興趣"]){
